...
for i in xrange(len(cipher_texts)):
  ...
  for c in xrange(min_len):
    all_caps = True
    for r in res:
      if (r[c] < 'A' or r[c] > 'Z') and r[c] != strxor(" ", " "):
        all_caps = False
        break
    if all_caps:
      for k, v in res.iteritems():
        texts[v][c] = strxor(k[c], " ")
      texts[i][c] = " "
...
update(2, "___ n_ce thing about Keey__q i_______e cryptograph_r_ _a_ drive a lot of fancy ca___- Dan _____")
update(0, "we can factor the number __ wi_______tum computers_ _e_c_n also factor the number___ with _____")
update(3, "the ciphertext produced by a w_______ryption algorithm looks as good as ciphertext produce_____")
update(1, "euler would probably enjoy tha_______is theorem becomes a corner stone of crypto - Annonymous__")
update(2, "the nice thing about Keeyloq i_______e cryptographers can drive a lot of fancy cars - Dan Boneh")
